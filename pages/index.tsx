  ...
  return (
    <div className="mt-12 flex flex-col items-center justify-center font-['Inter']">
      <div className="relative inline-flex w-full items-center justify-center">
        ...
        <input
          type="text"
          className="w-1/3 rounded-lg rounded-tr-3xl border border-slate-200 px-4 py-2 pl-10 text-slate-500 shadow-xl shadow-slate-50 outline-none ring-slate-200 transition duration-500 ease-in-out hover:border-slate-300 hover:shadow-slate-100 focus:border-slate-400 focus:ring active:bg-slate-100 active:ring"
      ...
      <div className="mt-12 flex w-9/12 flex-row flex-wrap items-center justify-center gap-3">
        {restaurants
          .filter((restaurant) =>
            restaurant.restaurant.includes(search.toLowerCase())
          )
          .map((restaurant) => (
